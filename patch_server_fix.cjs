...
// I might have deleted too much. Let's fix server.ts
// I'll rewrite the whole block from 220 to 290
const correctBlock = `
...
// Find everything from app.post('/api/admin/bot-session/submit'... to app.get('/api/admin/stats'...
code = code.replace(
    /app\.post\('\/api\/admin\/bot-session\/submit', async \(req, res\) => \{[\s\S]*?(?=app\.get\('\/api\/admin\/stats')/,
    correctBlock + '\n'
);
